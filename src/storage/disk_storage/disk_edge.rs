...
impl DiskEdge{
    /// Constructs a new `DiskEdge`. All arguments are **copied** (all are `u64`, which is `Copy`).
    ///
    /// `weight_offset` provides the byte offset of the weight data in `data.bin` so it can be located.
    /// `weight_len` specifies the length of the weight data in bytes to know how much to read.
    /// `node` specifies the target node identifier this edge connects to.
