    ...
    B: StorageBackend<W>,
{
    ...
    ///
    /// # Errors
    /// Returns `GraphErrors::NodeNotFound` if any specified `source` or `target` node does not exist.
    pub fn bulk_add_edge(&mut self, edges: &[(K, K, W)]) -> Result<(), GraphErrors>{
    ...
    pub fn bulk_remove_edge(&mut self, edges: &[(K, K, W)]) -> Result<(), GraphErrors>{
        let mut hashed_edges: Vec<(u64, u64, W)> = Vec::with_capacity(MAX_CAPACITY_BULK);
    ...
    B: StorageBackend<u32>,
{
    ...
    ///
    /// # Errors
    /// Returns `GraphErrors::NodeNotFound` if any edge source or target node doesn't exist.
    ...
    pub fn bulk_remove_edge(&mut self, edges: &[(K, K)]) -> Result<(), GraphErrors>{
        let mut hashed_edges: Vec<(u64, u64, u32)> = Vec::with_capacity(MAX_CAPACITY_BULK);
    ...
    B: StorageBackend<u32>,
{
    ...
    ///
    /// # Errors
    /// Returns [`GraphErrors::NodeNotFound`] if any edge source or target node is missing.
    pub fn bulk_add_edge(&mut self, edges: &[(K, K)]) -> Result<(), GraphErrors>{
