...
fn test_ram_storage_edge_cases() {
    ...
    let node1 = storage.add_node();
    let node2 = storage.add_node();

    let edge = Edge::new(node2, &100);
    storage.add_edge_to_node(&node1, &edge);
...
fn test_remove_edge_by_target() {
    ...
    storage.remove_edge_by_target(&node1, &node2);
